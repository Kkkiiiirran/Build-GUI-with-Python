...
def Countdown(time):
    if time >= 0:
        ...
        setTimer_bttn.config(state=NORMAL)
        tkinter.messagebox.showinfo("Time Countdown", "Time's up")


def disable_timer():
    """Disable further input and check button."""
    hoursEntry.config(state=DISABLED, bg="orange", fg="white")
    minutesEntry.config(state=DISABLED, bg="orange", fg="white")
    secondsEntry.config(state=DISABLED, bg="orange", fg="white")
    setTimer_bttn.config(state=DISABLED, bg="black", fg="lightgray")
